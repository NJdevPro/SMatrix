 ...
 *
 * Author: Nicolas Janin (circa 1998)
 */
...
public:
	...
	SMatrix(const size_t l, const size_t c, const T val): _iL(l), _iC(c) {
		_v.resize(l * c, val);
	}

	SMatrix(const size_t l, const size_t c, const T tab[], const size_t tab_sz): _iL(l), _iC(c) {
        assert (tab_sz == l * c);
		_v.resize(l * c);
	...
	SMatrix row(const size_t l){
		return (l < _iC) ? submatrix(l, 0, l, _iC - 1) : nullptr;
	}

	/// Extract a column
	SMatrix column(const size_t c){
		return (c < _iC) ? submatrix(0, c, _iL - 1, c) : nullptr;
	}
	...
	SMatrix submatrix(SMatrix &sub, const size_t i1, const size_t j1, const size_t i2, const size_t j2){
		if (in_place){
...
private:
	...
	SMatrix in_place_submatrix(const size_t i1, const size_t j1, const size_t i2, const size_t j2){
